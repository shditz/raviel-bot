...
module.exports = {
  name: "colorize",
  aliases: ["reviva", "mewarnai"],
  description: "Memberi warna pada foto hitam putih secara otomatis menggunakan AI.",
  async execute(sock, m, args, { jid }) {
    ...
    if (!msg) {
      return await sock.sendMessage(jid, { 
        text: `⚠️ *MEMBUTUHKAN GAMBAR*\n\nSilakan kirim foto hitam putih dengan caption *!colorize* atau balas (reply) foto yang sudah ada dengan perintah *!colorize*.` 
      }, { quoted: m });
    }

    await sock.sendMessage(jid, { 
        text: `⏳ *MOHON TUNGGU*\n\nSedang memproses pewarnaan gambar menggunakan AI. Proses ini membutuhkan waktu sekitar 15-30 detik...` 
    }, { quoted: m });
    ...
    try {
      ...
      const responseColorize = await axios.post("https://puruboy-api.vercel.app/api/tools/reviva", 
        { url: publicUrl }, 
        { responseType: 'stream' }
      ).catch(err => {
      ...
      await new Promise((resolve) => {
        ...
        responseColorize.data.on('data', (chunk) => {
        ...
        responseColorize.data.on('end', () => resolve());
      });
      ...
      await sock.sendMessage(jid, {
        image: finalBuffer,
        caption: `✨ *BERHASIL DIWARNAI*\n\nFoto Anda telah berhasil diwarnai secara otomatis menggunakan teknologi AI Reviva.`
      }, { quoted: m });
      ...
      console.error(err);
      await sock.sendMessage(jid, { 
        text: `❌ *KESALAHAN PROSES*\n\nMaaf, terjadi kendala saat memproses gambar Anda. Pastikan gambar jelas dan tidak terlalu besar ukurannya.` 
      }, { quoted: m });
    }
  }
};
